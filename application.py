...
def page_areas():
    ...
    if len(query_db("select name from Areas where lower(name) = lower(trim('{}'))".format(area_input)).index):
        ...
        if typeOther:
            ...
            curr_discipline.append("Mixed Rock")
            curr_discipline.append("Mixed Ice")
            curr_discipline.append("Deep Water Solo")
            curr_discipline.append("Toprope")
            curr_discipline.append("Aid")
...
#display user stats
def user_stat():
    uname_sel = st.text_input('Search Users (Some sample users: Ashima, Daniel, Sasha, Jimmy(two in database), Dai)')
...
def user_ascents(userid):
    ...
    typeSport = st.checkbox("Sport", value=True, key=str(userid) + "Sport")
    typeTrad = st.checkbox("Traditional", value=True, key=str(userid) + "Trad")
    typeBoulder = st.checkbox("Boulder", value=True, key=str(userid) + "Boulder")
    typeOther = st.checkbox("Other", value=True, key=str(userid) + "Other")
    ...
    # Order ascents based on column choice
    order_choice = st.selectbox('Order Ascents By:', ["Name", "Area", "Discipline", "Difficulty", "Quality", "Date"], key=str(userid) + "Order")
...
def user_toDos(userid):
    curr_order = st.selectbox('Order To Do List By:', ["Name", "Area", "Discipline", "Difficulty", "Quality"], key=str(userid) + "toDo")
